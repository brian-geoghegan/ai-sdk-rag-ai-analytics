...
export async function POST(req: Request) {
  ...
  const result = await streamText({
    ...
    After answering the question, always display the list of chunks returned from getInformation tool in a numbered list format format with a title 'Chunks' and also display the similarity score above each chunk labeled 'sim score'.
    ...
    1. Similarity score: {sim score}
       {chunkData}
    ________________________________
    2. Similarity score: {sim score}
       {chunkData}
    ________________________________
`,
    tools: {
